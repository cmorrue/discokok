...
import (
	...
	"github.com/cmorrue/discok/tools"
	_ "github.com/go-sql-driver/mysql"
)

func InserProduct(p models.Product) (int64, error) {
...
func SelectProduct(p models.Product, choice string, page int, pageSize int, orderType string, orderField string) (models.ProductResp, error) {
	...
	case "K":
		join := " JOIN category ON Prod_CatregoryId = Categ_Id AND Categ_Path LIKE '%" + strings.ToUpper(p.ProdCategPath) + "%' "
		sentencia += join
		sentenciaCount += join
	}
	...
	if len(orderField) > 0 {
		...
		case "F":
			orderBy = " ORDER BY Prod_CreateAt "
		case "P":
	...
	for rows.Next() {
		...
		err := rows.Scan(&ProdId, &ProdTitle, &ProdDescription, &ProdCreatedAt, &ProdUpdated, &ProdPrice, &ProdCategoryId, &ProdStock)
		if err != nil {
			return Resp, err
		}
