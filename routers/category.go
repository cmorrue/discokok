...
func SelectCategories(body string, request events.APIGatewayV2HTTPRequest) (int, string) {
	...
	if len(request.QueryStringParameters["categId"]) > 0 {
		...
		if err != nil {
			return 500, "Ocurrio un error al intentar concertir en entero el valor " + request.QueryStringParameters["categId"]
		}
	} else {
	...
	if err2 != nil {
		return 400, "Ocurrio un error al intentar capturar de la categoria/s  > " + err2.Error()
	}
